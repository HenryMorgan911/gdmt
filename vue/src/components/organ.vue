   ...
   search: function(val) {
    ...
    methods: {
      ...
      handleEdit(index, row) {
        ...
        this.organ.name=row.name;
        this.value=row.pid;
      },
      handleDelete(row) {
        //console.log(row)
        this.axios.post("http://localhost:8081/Organ/delete",row).then(res=>{
          this.$message({
    ...
    handleCurrentChange: function(currentPage) {
      ...
      updata(){
          ...
          if(this.organ.id==this.value){
            this.value=0;
          }
          this.organ.pid=this.value
          //console.log(this.organ)
          this.axios.post("http://localhost:8081/Organ/update",this.organ).then(res=>{
            this.$message({
              message: '恭喜你，这是一条成功消息',
              type: 'success'
            })
